# Serkan Efe Erisev

Computer Engineering (Senior) • Cybersecurity & IT • AI/ML Projects  
📍 Cyprus / TR • 📧 destekserkan066@hotmail.com

## Live CV
- 🌐 CV Website: https://serkan-77.github.io/CV/

## About Me
I’m a senior Computer Engineering student focusing on **Cybersecurity (SOC / Pentest)** and **IT systems**.  
I also build software projects with Python, Java, JS/Node, React/React Native and work on AI/ML applications.

## Skills
- Cybersecurity, Networking, IT
- Python (Advanced), C/C++/C# (Intermediate), Java (Intermediate)
- HTML/CSS/JS, Node.js, React / React Native
- SQL & NoSQL
- Unity, 2D/3D Design

## Languages
- Turkish: Native
- English: Advanced
- German: Beginner

## Experience (Highlights)
- TURKCELL (Arday Bilişim) — Database Solution Engineer / Consultant (Dec 2021 – Apr 2022)
- PulseTRonics — Systems Engineering Intern (Jul 2023 – Sep 2023)
- Just Kıbrıs — Full Stack Developer & Debugging Engineer (Apr 2024 – Jun 2024)
- Ekinoks Yazılım — AI & ML Engineer (Jul 2025 – Aug 2025)

## Certificates
- CISCO Junior Cybersecurity Analyst (2025)
- CISCO Network Technician (2025)
- TryHackMe: Intro to Cybersecurity (2024)
- TryHackMe: Cybersecurity 101 (2024)
- Udemy: Ethical Hacker (2023) + Level 2 (2023)
- Udemy: Python (2024–2025)
- Udemy: Arduino Fundamentals (2022)

## Projects
- GitHub Projects: https://github.com/Serkan-77
- Cybersecurity Writeups (LinkedIn / HackMyVm)
- Pwnagotchi (Raspberry Pi, Python, Wi-Fi Security)
- Python Keylogger (Educational)
- MAC/IP Changer (Python)
- System Info Scanner / Spoofer (Python)
- Multiple Unity 2D game prototypes
- Arduino Radar & Line Follower Robot
- Gym Management System (React/React Native/Node/Firebase)
- Web-based Medicine Tracking System

## Links
- GitHub: https://github.com/Serkan-77
- LinkedIn: https://www.linkedin.com/in/serkan-efe-erisev/

## This is New !!
- GitHub: https://github.com/Serkan-77/Face_AI_Attendance_System
- GitHub: https://github.com/Serkan-77/QR_Attendance_System
- Github: https://github.com/Serkan-77/Location-Attendace-System
